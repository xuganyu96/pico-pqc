 ...
 * TODO: good RNG https://www.raspberrypi.com/documentation/pico-sdk/high_level.html#group_pico_rand_1ga97a9544b527a3ba865ab70142bdd5d1b
 */
...
static OQS_STATUS kem_test_correctness(const char *method_name)
{
    ...
    if (!pk) {
        printf("Requested %zu/%zu bytes for pk\n", 
            kem->length_public_key, TRY_MALLOC_MAX);
        goto err;
    }
    ...
    if (!sk) {
        printf("Requested %zu/%zu bytes for sk\n", 
            kem->length_secret_key, TRY_MALLOC_MAX);
        goto err;
    }
    ...
    if (!ct) {
        printf("Requested %zu/%zu bytes for ct\n", 
            kem->length_ciphertext, TRY_MALLOC_MAX);
        goto err;
    }
    ...
    if (!ss) {
        printf("Requested %zu/%zu bytes for ss\n", 
            kem->length_shared_secret, TRY_MALLOC_MAX);
        goto err;
    }
...
cleanup:
    if (pk) OQS_MEM_insecure_free(pk);
    if (ct) OQS_MEM_insecure_free(ct);
    if (sk) OQS_MEM_secure_free(sk, kem->length_secret_key);
    if (ss) OQS_MEM_secure_free(ss, kem->length_shared_secret);
    if (ss_cmp) OQS_MEM_secure_free(ss_cmp, kem->length_shared_secret);
    OQS_KEM_free(kem);
