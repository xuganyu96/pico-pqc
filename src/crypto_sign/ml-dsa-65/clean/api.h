...
inline int PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_inline(
  uint8_t *sig, size_t *siglen,
  const uint8_t *msg, size_t msglen,
  const uint8_t *sk
) {
  return PQCLEAN_MLDSA65_CLEAN_crypto_sign_signature_ctx(sig, siglen, msg, msglen, NULL, 0, sk);
}

inline int PQCLEAN_MLDSA65_CLEAN_crypto_sign_verify_inline(
  const uint8_t *sig, size_t siglen,
  const uint8_t *msg, size_t msglen,
  const uint8_t *pk
) {
  return PQCLEAN_MLDSA65_CLEAN_crypto_sign_verify_ctx(sig, siglen, msg, msglen, NULL, 0, pk);
}
#endif
